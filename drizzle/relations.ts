...
export const konzumiranahranaRelations = relations(
  konzumiranahrana,
  ({ one }) => ({
    hrana: one(hrana, {
      fields: [konzumiranahrana.hranaId],
      references: [hrana.hranaId],
    }),
    korisnik: one(korisnik, {
      fields: [konzumiranahrana.korisnikId],
...
export const odradjeneaktivnostiRelations = relations(
  odradjeneaktivnosti,
  ({ one }) => ({
    aktivnost: one(aktivnost, {
      fields: [odradjeneaktivnosti.aktivnostId],
      references: [aktivnost.aktivnostId],
    }),
    korisnik: one(korisnik, {
      fields: [odradjeneaktivnosti.korisnikId],
      references: [korisnik.korisnikId],
    }),
  }),
);
